...
static bool bpf_storage_filter_is_valid_access(int off, int size, enum bpf_access_type type,
					  const struct bpf_prog *prog,
					  struct bpf_insn_access_aux *info)
{
	if (off < 0 || off + size > sizeof(struct bpf_storage_ctx))
		return false;

	if (type != BPF_READ)
		return false;

	if (off == offsetof(struct bpf_storage_ctx, bio)) {
		if (size != sizeof(__u64))
			return false;

		info->reg_type = PTR_TO_BTF_ID | PTR_MAYBE_NULL;
		info->btf = btf_vmlinux;
		info->btf_id = btf_tracing_ids[BTF_TRACING_TYPE_BIO];
	} else {
		return false;
	}
